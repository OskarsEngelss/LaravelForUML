...
<body style="display: flex; justify-content: center; flex-wrap: wrap; font-family: monospace; background-color: #ebebeb">
    ...
    >
        ...
        <label style="font-size: 2em; display: flex; justify-content: center; align-items: center; flex-direction: column; padding: 5px 10px;">
            Amount:
            <input name="amount" type="number" style="width: 200px; height: 30px; border-radius: 12px; border: 1px solid black;" value="{{ old('amount', $expense->amount) }}">
        </label>
        <label style="font-size: 2em; display: flex; justify-content: center; align-items: center; flex-direction: column; padding: 5px 10px;">
            Date:
            <input name="date" type="date" style="width: 200px; height: 30px; border-radius: 12px; border: 1px solid black;" value="{{ old('amount', $expense->date) }}">
        </label>
        <label style="font-size: 2em; display: flex; justify-content: center; align-items: center; flex-direction: column; padding: 5px 10px;">
            Category:
            <select name="category_id" style="font-family: monospace; width: 200px; height: 35px; border-radius: 12px; border: 1px solid black;" id="fruits" name="fruits">
                @foreach ($categories as $category)
                    <option style="font-family: monospace; color: {{ $category->hex_color }}" value="{{ $category->id }}" 
        ...
        <label style="font-size: 2em; display: flex; justify-content: center; align-items: center; flex-direction: column; padding: 5px 10px;">
            Notes:
            <input name="notes" type="string" style="width: 200px; height: 30px; border-radius: 12px; border: 1px solid black;" value="{{ old('amount', $expense->notes) }}">
        </label>
